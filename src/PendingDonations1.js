...
const PendingDonations1 = () => {
  const [pendingDonations, setPendingDonations] = useState([]);
  const [filteredDonations, setFilteredDonations] = useState([]);
  const [searchTerm, setSearchTerm] = useState('');
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const [processingId] = useState(null);
  ...
  toast.info('Login as NGO to approve donations', {
    position: "top-right",
    autoClose: 3000,
  ...
  setTimeout(() => {
    window.location.href = "/login?from=pendingdonations";
 // Adjust the route if your login page is different
  }, 1500);
};
...
export default PendingDonations1;